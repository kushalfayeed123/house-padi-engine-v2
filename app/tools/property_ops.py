...
class CreatePropertyInput(BaseModel):
    ...
    base_price: float = Field(..., description="The base price listing value of the property asset. Pass numbers only.")
    ...
    def coerce_numeric_string(cls, v: Any) -> float:
        """Coerces stringified numbers generated by local models into true floats."""
        if isinstance(v, str):
            # Clean out currency symbols, commas, or spaces if the model hallucinated them
            sanitized = v.replace("₦", "").replace("$", "").replace(",", "").strip()
            try:
                return float(sanitized)
            except ValueError:
                raise ValueError(f"Failed to parse base_price string '{v}' into a valid float.")
        return v
